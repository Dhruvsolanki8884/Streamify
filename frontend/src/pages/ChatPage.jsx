...
const MsgMenu = ({ message, isMine, onReact, onClose, onReply, myRT }) => {
  ...
  return (
    <div ref={ref} className={`absolute z-50 bottom-full mb-2 flex flex-col rounded-2xl shadow-2xl overflow-hidden bg-[#233138] border border-[#2a3942] min-w-[190px] ${isMine ? "right-0" : "left-0"}`}>
      ...
      <button onClick={() => {
        navigator.clipboard?.writeText(message.text || "").then(() => { setCopied(true); setTimeout(onClose, 1200); });
      }} className="flex items-center gap-3 px-4 py-3 text-sm text-[#e9edef] hover:bg-white/5">
        {copied ? <CheckIcon className="size-4 text-[#00a884]" /> : <CopyIcon className="size-4 text-[#8696a0]" />}
        {copied ? "Copied!" : "Copy"}
      </button>
...
const CallLogBubble = ({ message }) => {
  ...
  return (
    <div className="flex justify-end w-full px-2 mb-1">
      <div className="bg-[#005c4b] rounded-[18px] rounded-tr-[4px] px-3 py-2.5 flex items-center gap-3" style={{ minWidth: 180, maxWidth: 260 }}>
        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0">
          {isVideo ? <VideoIcon className="size-5 text-white/70" /> : <PhoneIcon className="size-5 text-white/70" />}
        </div>
        <div className="flex-1 min-w-0">
  ...
  return (
    <div className={`flex w-full mb-1 px-2 ${isMine ? "justify-end" : "justify-start"}`}>
      <div className="relative group" style={{ maxWidth: "min(78%, 300px)" }}>
        ...
        {showMenu && (
          <MsgMenu message={message} isMine={isMine} myRT={myRT} onReact={onReact}
            onReply={(m) => { setReplyTo(m); }} onClose={() => setShowMenu(false)} />
        )}
        ...
        <div
          ...
          onTouchMove={() => clearTimeout(lpt.current)}
          onContextMenu={(e) => { e.preventDefault(); setShowMenu(true); }}
          className={`relative px-3 pt-2 shadow-sm
            ${isMine ? "bg-[#005c4b] text-white rounded-[18px] rounded-tr-[4px]" : "bg-[#202c33] text-[#e9edef] rounded-[18px] rounded-tl-[4px]"}`}
          style={{ wordBreak: "break-word" }}
        >
          {!isMine && message.user?.name && (
          ...
          {isShort ? (
            ...
            /* LONG: text flows, time is absolutely positioned, spacer prevents overlap */
            <div className="pb-5">
              <p className="text-[14.5px] leading-[1.45]">{message.text}</p>
              <div className="absolute bottom-1.5 right-2.5 flex items-center gap-0.5">
                <span className="text-[10px]" style={{ color: isMine ? "rgba(255,255,255,0.5)" : "rgba(233,237,239,0.4)" }}>{time}</span>
...
const OnlineStatus = ({ userId }) => {
  ...
  return <span className="text-[11px]" style={{ color: online ? "#00a884" : "#8696a0" }}>{online ? "online" : "offline"}</span>;
};
...
const Typing = ({ channel, myId }) => {
  ...
  return (
    <div className="flex justify-start px-3 pb-2">
      <div className="bg-[#202c33] rounded-[18px] rounded-tl-[4px] px-3 py-2 flex gap-1 items-center">
        {[0, 1, 2].map(i => (
          <span key={i} className="w-1.5 h-1.5 rounded-full bg-[#8696a0]"
...
const ReplyBanner = ({ replyTo, onCancel }) => {
  ...
  return (
    <div className="flex items-start gap-2 px-3 py-2 bg-[#1f2c34] border-l-4 border-[#00a884] mx-2 mb-1 rounded-lg">
      ...
      <button onClick={onCancel} className="p-1 text-[#8696a0]"><XIcon className="size-4" /></button>
    </div>
  );
};
...
const MsgInput = ({ channel }) => {
  ...
  return (
    <div className="bg-[#0b141a] flex-shrink-0">
      {replyTo && <ReplyBanner replyTo={replyTo} onCancel={() => setReplyTo(null)} />}
      <div className="flex items-end gap-2 px-2 py-2">
        <button className="w-9 h-9 flex items-center justify-center text-[#8696a0] flex-shrink-0">
        ...
        <div className="flex-1 bg-[#2a3942] rounded-[22px] flex items-end px-3 py-2 min-h-[44px]">
          <textarea ref={ta} value={text} rows={1}
            ...
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }}
            placeholder="Message"
            className="flex-1 bg-transparent text-[#e9edef] placeholder-[#8696a0] resize-none outline-none leading-[1.4] max-h-[120px] overflow-y-auto"
            style={{ fontSize: "16px" }}
          />
        </div>
        <div className="flex-shrink-0">
          {text.trim()
            ? <button onClick={send} disabled={busy} className="w-11 h-11 bg-[#00a884] rounded-full flex items-center justify-center active:scale-90 disabled:opacity-60 shadow-lg">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" fill="white" /></svg>
              </button>
            : <button className="w-11 h-11 bg-[#00a884] rounded-full flex items-center justify-center active:scale-90 shadow-lg">
...
const ChatHeader = ({ user, userId, onVideo, onVoice }) => {
  ...
  return (
    <div className="flex items-center gap-1 px-1 bg-[#202c33] border-b border-[#2a3942] flex-shrink-0 z-20"
      style={{ paddingTop: "max(10px, env(safe-area-inset-top, 0px))", paddingBottom: "10px" }}>
      <button onClick={() => navigate(-1)}
        className="p-2 rounded-full text-[#aebac1] hover:bg-white/10 active:bg-white/20 flex-shrink-0">
      ...
      <div className="flex items-center flex-shrink-0">
        <button onClick={onVoice} className="p-2.5 rounded-full text-[#aebac1] hover:bg-white/10 active:scale-90 transition-all">
          <PhoneIcon className="size-5" />
        </button>
        <button onClick={onVideo} className="p-2.5 rounded-full text-[#aebac1] hover:bg-white/10 active:scale-90 transition-all">
...
/* ── Inner layout — needs Channel context for OnlineStatus ── */
const ChatInner = ({ channel, targetUser, targetUserId, onVideo, onVoice }) => {
  const listRef = useRef(null);

  // Scroll to bottom on mount
  useEffect(() => {
    if (listRef.current) {
      setTimeout(() => { listRef.current.scrollTop = listRef.current.scrollHeight; }, 200);
    }
  }, []);
...
const ChatPage = () => {
  ...
  return (
    <RC.Provider value={{ replyTo, setReplyTo }}>
      <style>{`
        @keyframes tb{0%,60%,100%{transform:translateY(0);opacity:.4}30%{transform:translateY(-4px);opacity:1}}
        /* Hide ALL Stream default chrome — we render everything ourselves */
        .str-chat__channel-header,.str-chat__input-flat,.str-chat__message-input,.str-chat__message-notification{display:none!important}
        .str-chat,.str-chat__container,.str-chat__channel{height:100%!important;min-height:0!important;background:transparent!important;display:flex!important;flex-direction:column!important}
        ...
        /* Message list scrolls inside its container */
        .str-chat__list,.str-chat__message-list,.str-chat__reverse-infinite-scroll{background:transparent!important;padding:4px 0!important}
        .str-chat__li,.str-chat__message-simple{padding:0!important;margin:0!important}
        .str-chat__message-reactions-button,.str-chat__reaction-selector,.str-chat__message-actions-box,.str-chat__message-options{display:none!important}
        /* Window fills full height */
        .str-chat__channel{flex:1!important;min-height:0!important}
      `}</style>

      {/*
        KEY FIX: Use 100dvh which auto-shrinks when keyboard opens on mobile.
        The flex column layout means:
        - Header: fixed height, never scrolls
        - Messages: flex-1, scrolls
        - Input: fixed height, stays above keyboard
      */}
      <div
      ...
      >
        <Chat client={chatClient}>
          <Channel channel={channel} Message={CustomMessage} DateSeparator={DateSep} returnAllReadData>
            <Window hideOnThread>
              {/*
                We render our own layout INSIDE Window so we get Channel context
                but control the structure ourselves
              */}
